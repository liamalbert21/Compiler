...
# Overview
This project demonstrates an expression compiler implemented using modern C++ features (C++20). Currently, the program is capable of the following:
...
`type` serves as the primary differentiator of tokens throughout the pipeline. When the active parser needs to match tokens at certain precedence levels or the lexer needs to distinguish numeric types, it uses this field to do so.
...
Most tokens implementations include an additional field: the lexeme. While literals represent the underlying data, lexemes provide a textual representation of data. This field is unnecessary for this project, however, as the overloaded insertion operator (`<<`) already handles piping raw data to streams.
...
You can customize where the actual token values appear (relative to the left-hand side of your console) by varying the parameter `right_just`. Below is the print result with `right_just = 30` and input `( 8 * 3.4 ) / 9! + [ 22 - -7 ]`:
...
The tokenization logic is rather straightforward. When a new token is first detected (i.e. when `getToken` is called), the lexer attempts to deduce its type with a switch statement (`guessTokenType`). Should the result be a complete type, the literal is assigned appropriately, and the token window [m_start, m_current] clamps to [m_current, m_current] as a means to prepare the lexer for the next token. Whitespace is ignored at the callsite (`tokenize`) to simplify edge cases in the extraction logic.

Numbers are slightly more complex. The compiler supports both integers and double-precision numbers, so they must be differentiated. To do so, the lexer begins by "reguessing" the token type with `init_guess`, provided earlier by `guessTokenType`.
...
## Expressions
Expression representation is realized through inheritance and the visitor pattern. `Binary`, `Unary`, `Primary` and `Grouping` all inherit from an abstract base `Expr` which provides both a structural framework for printing and evaluation and allows derived classes to be interpreted under a single class.

Within `Expr`, a abstract visitor class `Visitor` is defined that consolidates visit methods, through which derived classes perform certain functionality. The functionality in question is overriden by classes that inherit from visitor; passing these derived classes overriden accept calls forwards execution to the visitor.
...
Note that it is vital to provide a virtual default destructor inside `Expr`, as you cannot delete a derived expression through the base otherwise. It must be public (not protected/private) to allow for explicit calls to `~Expr`.

Alternative approaches to this model include aliasing a `std::variant` that holds custom expression types as `Expr` and performing runtime checks on active type. The inheritance model has no particular advantage over others and was chosen solely because of stylistic preference.

## Parser
The parser is undeniably the most sophisticated pipeline in the system. It converts a tokenized list (typically provided by a lexer) into an abstract syntax tree (AST), which is in essence a hierarchial expression diagram where nodes represent operands (nonterminals) and leaves represent terminal expressions.

### Theory
ASTs handle order of operations by depth, as in the deeper the expression, the earlier its evaluation will commence. To understand this, consider the following input:
...
A "`/`" is then found, signaling to the parser that another complete expression must exist to the right. Afterwards, it identifies a "`9`" and the factorial symbol "`!`", which it knows to group with the preceding expression (terminal). This pattern continues until the parser either has iterated through the entire token list or can no longer grow the AST due to illegal syntax.
...
Similar to `Lexer`, `Parser` maintains an internal state variable that knows whether any errors occurred, and there are currently two ways to set its state to `FAIL`:
...
This allows the logger to illuminate multiple errors (as opposed to just one) when provided inputs such as `+ 17 38`. Since one can rectify numerous errors in a single run, the effort entailed in debugging is significantly reduced.
## Error Handling
...
To accomplish this, an explicit template specialization (ETS) of `Error` is declared and defined within the source file of each associated pipeline. The placement was chosen to preserve encapsulation, and a constraint is applied to all specializations that requires the argument to inherit `Pipeline`. There is no restriction on access specifiers.
...
It should also be noted that each member function ends with a call to a public error wrapper tied to the class argument of the `Error` specialization.
...
Enclosing the container within `std::reference_wrapper` was an alternative in mind, but introduced unnecessary verbosity. For instance, the snippet
